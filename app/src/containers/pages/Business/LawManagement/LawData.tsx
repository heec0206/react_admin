...
const LawData = props => {
  ...
  //조회
  const [SearchMode, setSearchMode] = useState<[string, string]>();
  const setSearchCall = useCallback(x => {
    console.log('조회 결과 :', x);
    setSearchMode(x);
    console.log('search call', SearchMode);
    ...
    const result = axios
      ...
      .then(function (response) {
        ...
        //gridRef.current.api.sizeColumnsToFit();
        gridRef.current.api.deselectAll();
        gridRef.current.api.sizeColumnsTDailyStatusMenuoFit();
      });
  }, []);
